...
const wasteColors = {
  organic: { bg: "bg-[#4CAF50]/10", text: "text-[#4CAF50]", dot: "bg-[#4CAF50]" },
  plastic: { bg: "bg-[#2196F3]/10", text: "text-[#2196F3]", dot: "bg-[#2196F3]" },
  paper: { bg: "bg-[#D2B48C]/10", text: "text-[#D2B48C]", dot: "bg-[#D2B48C]" },
  metal: { bg: "bg-[#78909C]/10", text: "text-[#78909C]", dot: "bg-[#78909C]" },
  ewaste: { bg: "bg-[#7B1FA2]/10", text: "text-[#7B1FA2]", dot: "bg-[#7B1FA2]" },
  glass: { bg: "bg-[#009688]/10", text: "text-[#009688]", dot: "bg-[#009688]" },
  hazardous: { bg: "bg-[#FF5722]/10", text: "text-[#FF5722]", dot: "bg-[#FF5722]" },
};
...
const PickupHistory = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      api.get(`/pickups?limit=20&page=${page}${statusFilter ? `&status=${statusFilter}` : ""}`),
  });
        ...
        {isLoading ? (
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-lg font-medium text-muted-foreground">No pickups found</p>
              <p className="text-sm text-muted-foreground/70 mt-1">
                {statusFilter ? "Try a different filter" : "Schedule your first pickup"}
              </p>
            </CardContent>
          </Card>
        ) : (
          <motion.div variants={container} initial="hidden" animate="show" className="space-y-3">
            {pickups.map((pickup) => {
              ...
              const wc = wasteColors[pickup.wasteType] || { bg: "bg-gray-100", text: "text-gray-600", dot: "bg-gray-400" };
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="font-semibold capitalize">{pickup.wasteType} Waste</span>
                            <Badge variant="secondary" className={`${wc.bg} ${wc.text} text-xs`}>
                              {pickup.wasteType}
                            </Badge>
                          </div>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <Badge variant="secondary" className={`${sc.color} gap-1.5`}>
